...
type Manager struct {
	...
	// processes (survivors of a previous q-ui run) has already run.
	swept bool
}
...
// sweepOrphansLocked kills mtg processes left running by a previous q-ui run,
// exactly once per process lifetime and before any of our own mtg are started.
// Because q-ui owns every mtg process, anything alive at this point is an orphan
...
// — which q-ui always emits and scrapes for traffic (see scrapeTraffic).
