...
#
# Installs the released q-ui binary into a container WITHOUT a database, runs
# q-ui-firstboot.sh, and asserts:
#   * fresh random credentials are generated (no admin/admin)
#   * /etc/q-ui/credentials.txt (600) and /etc/motd are written
...
docker run --rm \
    -v "${REPO_ROOT}/deploy/firstboot/q-ui-firstboot.sh:/root/q-ui-firstboot.sh:ro" \
    -e DEBIAN_FRONTEND=noninteractive \
    "$IMAGE" bash -euo pipefail -c '
        apt-get update -qq
        apt-get install -y -qq curl tar openssl ca-certificates jq > /dev/null

        echo "--- installing released q-ui binary (no DB, no systemd) ---"
        ...
        if ! curl -4 --fail --location --silent --show-error \
            ...
            -o "${tmp}/x.tar.gz" \
            "https://github.com/${REPO}/releases/download/${VER}/q-ui-linux-${ARCH}.tar.gz"; then
            echo "FAIL: cannot download q-ui-linux-${ARCH}.tar.gz (${VER})" >&2; exit 1
        fi
        test -s "${tmp}/x.tar.gz" || { echo "FAIL: downloaded tarball is empty"; exit 1; }
        tar -xzf "${tmp}/x.tar.gz" -C /usr/local/
        chmod +x /usr/local/q-ui/q-ui
        install -m 755 /root/q-ui-firstboot.sh /usr/local/q-ui/q-ui-firstboot.sh

        # Guarantee a clean slate (the image must never ship a DB).
        rm -f /etc/q-ui/q-ui.db /etc/q-ui/.firstboot-done

        echo "--- run 1: generate per-instance credentials ---"
        /usr/local/q-ui/q-ui-firstboot.sh

        test -f /etc/q-ui/.firstboot-done || { echo "FAIL: sentinel not created"; exit 1; }
