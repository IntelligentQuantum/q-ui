...
rm -f /etc/q-ui/q-ui.db /etc/q-ui/q-ui.db-* 2> /dev/null || true
rm -f /etc/q-ui/install-result.env /etc/q-ui/credentials.txt 2> /dev/null || true
rm -f /etc/q-ui/.firstboot-done 2> /dev/null || true
...
rm -rf /var/lib/q-ui /var/log/q-ui/* 2> /dev/null || true
apt-get clean || true
...
fail=0
for f in /etc/q-ui/q-ui.db /etc/q-ui/credentials.txt /etc/q-ui/install-result.env /etc/q-ui/.firstboot-done; do
    if [ -e "$f" ]; then
