...
# firewall is intentionally NOT enforced by default because q-ui opens Xray
...
cat > /etc/ssh/sshd_config.d/99-q-ui-hardening.conf << 'EOF'
# q-ui golden image hardening (AWS Marketplace scanner compliance)
PasswordAuthentication no
PermitRootLogin prohibit-password
KbdInteractiveAuthentication no
ChallengeResponseAuthentication no
EOF
chmod 644 /etc/ssh/sshd_config.d/99-q-ui-hardening.conf
