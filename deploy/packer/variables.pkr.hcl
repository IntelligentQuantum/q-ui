// Input variables for the q-ui golden-image build.
// See README.md for usage. Override with -var / -var-file or env (PKR_VAR_*).

variable "qui_version" {
  type        = string
  description = "q-ui release tag to install, e.g. v3.3.1. 'latest' resolves the newest GitHub release at build time."
  default     = "latest"
}
...
variable "qui_arch" {
  ...
  default     = "amd64"
  validation {
    condition     = contains(["amd64", "arm64"], var.qui_arch)
    error_message = "The qui_arch value must be 'amd64' or 'arm64'."
  }
}
...
variable "instance_type" {
  type        = string
  description = "EC2 instance type used to build the AMI. Must match qui_arch (e.g. t3.small for amd64, t4g.small for arm64/Graviton)."
  default     = "t3.small"
}
...
variable "source_ami_filter_name" {
  type        = string
  description = "Override for the Canonical Ubuntu base AMI name filter. Empty ⇒ derived from qui_arch (latest patched 24.04 LTS for that arch)."
  default     = ""
}
...
variable "qemu_iso_url" {
  type        = string
  description = "Override for the Ubuntu cloud image used as the qemu base disk. Empty ⇒ derived from qui_arch (amd64/arm64 cloud image)."
  default     = ""
}
...
variable "qemu_efi_code" {
  type        = string
  description = "Path to the arm64 UEFI code firmware (AAVMF). Only used when qui_arch=arm64."
  default     = "/usr/share/AAVMF/AAVMF_CODE.fd"
}

variable "qemu_efi_vars" {
  type        = string
  description = "Path to the arm64 UEFI vars firmware template (AAVMF). Only used when qui_arch=arm64."
