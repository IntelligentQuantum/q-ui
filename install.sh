...
install_q-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        tag_version=$(curl -Ls "https://api.github.com/repos/IntelligentQuantum/q-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${yellow}Trying to fetch version with IPv4...${plain}"
            tag_version=$(curl -4 -Ls "https://api.github.com/repos/IntelligentQuantum/q-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
            if [[ ! -n "$tag_version" ]]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/IntelligentQuantum/q-ui/releases/download/${tag_version}/q-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    curl -4fLRo /usr/bin/q-ui-temp https://raw.githubusercontent.com/IntelligentQuantum/q-ui/main/q-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ -e ${xui_folder}/ ]]; then
        ...
        # Kill any leftover mtg (MTProto) sidecars. q-ui runs them outside its own
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/q-ui https://raw.githubusercontent.com/IntelligentQuantum/q-ui/main/q-ui.rc
        if [[ $? -ne 0 ]]; then
