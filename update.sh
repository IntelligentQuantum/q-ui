...
prompt_and_setup_ssl() {
    ...
    case "$ssl_choice" in
        ...
        2)
            ...
            if [[ $release == "alpine" ]]; then
                rc-service q-ui restart > /dev/null 2>&1
            else
                systemctl restart q-ui > /dev/null 2>&1
            fi
...
update_q-ui() {
    ...
    if [[ -e ${xui_folder}/ ]]; then
        ...
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/q-ui" ]; then
                rc-service q-ui stop > /dev/null 2>&1
                rc-update del q-ui > /dev/null 2>&1
                echo -e "${green}Removing old service unit version...${plain}"
                rm -f /etc/init.d/q-ui > /dev/null 2>&1
            else
            ...
            if [ -f "${xui_service}/q-ui.service" ]; then
                ...
                rm q-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
                _fail "ERROR: q-ui systemd unit not installed."
            fi
        fi
    ...
    echo -e "${green}Installing new q-ui version...${plain}"
    tar zxvf q-ui-linux-$(arch).tar.gz > /dev/null 2>&1
    rm q-ui-linux-$(arch).tar.gz -f > /dev/null 2>&1
    cd q-ui > /dev/null 2>&1
    chmod +x q-ui > /dev/null 2>&1
    ...
    if [[ $release == "alpine" ]]; then
        ...
        chmod +x /etc/init.d/q-ui > /dev/null 2>&1
        chown root:root /etc/init.d/q-ui > /dev/null 2>&1
        rc-update add q-ui > /dev/null 2>&1
        rc-service q-ui start > /dev/null 2>&1
    else
    ...
    echo -e "${green}q-ui ${tag_version}${plain} updating finished, it is running now..."
    echo -e ""
    echo -e "┌───────────────────────────────────────────────────────┐
