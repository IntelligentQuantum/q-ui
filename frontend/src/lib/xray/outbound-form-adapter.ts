...
function dnsToWire(s: DnsOutboundFormSettings)
{
    ...
    if (s.rewriteNetwork)
    {
        result.rewriteNetwork = s.rewriteNetwork;
    }
    if (s.rewriteAddress)
    {
        result.rewriteAddress = s.rewriteAddress;
    }
    if (s.rewritePort)
    {
        result.rewritePort = s.rewritePort;
    }
    if (s.userLevel)
    {
        result.userLevel = s.userLevel;
    }
    if (s.rules.length > 0)
    {
        result.rules = s.rules.map(dnsRuleToWire);
    }
    return result;
}
...
{
    if (!MUX_PROTOCOLS.has(values.protocol))
    {
        return false;
    }
...
export function formValuesToWirePayload(values: OutboundFormValues): WireOutboundPayload
{
    ...
    if (values.streamSettings)
    {
        ...
        else
        {
            ...
            if (sockopt)
            {
                result.streamSettings = { sockopt };
            }
        }
    }

    if (values.sendThrough)
    {
