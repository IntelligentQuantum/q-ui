...
): Record<string, unknown>
{
    ...
    if (out.noGRPCHeader !== true)
    {
        delete out.noGRPCHeader;
    }
    if (out.noSSEHeader !== true)
    {
        delete out.noSSEHeader;
    }
    if (out.serverMaxHeaderBytes === 0)
    {
        delete out.serverMaxHeaderBytes;
    }
    if (out.uplinkChunkSize === 0)
    {
        delete out.uplinkChunkSize;
    }
    ...
    if (mode === 'stream-one')
    {
        for (const key of PACKET_UP_FIELDS)
        {
            delete out[key];
        }
        for (const key of STREAM_UP_SERVER_FIELDS)
        {
            delete out[key];
        }
    }
...
): Record<string, unknown> | undefined
{
    ...
    if (out.domainStrategy === 'AsIs')
    {
        delete out.domainStrategy;
    }
    if (out.addressPortStrategy === 'none')
    {
        delete out.addressPortStrategy;
    }
    if (nonEmptyString(out.dialerProxy) === false)
    {
        delete out.dialerProxy;
    }
    if (nonEmptyString(out.interface) === false)
    {
    ...
    if (isRecord(he))
    {
        ...
        if (heOut.tryDelayMs === 0)
        {
            delete heOut.tryDelayMs;
        }
        if (heOut.prioritizeIPv6 === false)
        {
            delete heOut.prioritizeIPv6;
        }
        if (heOut.interleave === 1)
        {
            delete heOut.interleave;
        }
        if (heOut.maxConcurrentTry === 4)
        {
            delete heOut.maxConcurrentTry;
        }
    ...
    if (nonEmptyString(out.tcpcongestion) === false)
    {
        delete out.tcpcongestion;
    }
