...
export function genVmessLink(input: GenVmessLinkInput): string
{
    ...
    if (tls === 'tls' && stream.security === 'tls')
    {
        ...
        if (tlsSettings.serverName.length > 0)
        {
            obj.sni = tlsSettings.serverName;
        }
        if (tlsSettings.settings.fingerprint.length > 0)
        {
            obj.fp = tlsSettings.settings.fingerprint;
        }
        if (tlsSettings.alpn.length > 0)
        {
            obj.alpn = tlsSettings.alpn.join(',');
        }
...
export function genVlessLink(input: GenVlessLinkInput): string
{
    ...
    if (security === 'tls')
    {
        ...
        if (stream.security === 'tls')
        {
            ...
            if (tls.serverName.length > 0)
            {
                params.set('sni', tls.serverName);
            }
            if (tls.settings.echConfigList.length > 0)
            {
            ...
            if (stream.network === 'tcp' && flow.length > 0)
            {
    ...
    else if (security === 'reality')
    {
        ...
        reality.target?.split(':')[0];
            ...
            if (reality.shortIds.length > 0)
            {
                params.set('sid', reality.shortIds[0]);
            }
            if (reality.settings.spiderX.length > 0)
            {
...
export function genShadowsocksLink(input: GenShadowsocksLinkInput): string
{
    ...
    if (isSS2022)
    {
        passwords.push(settings.password);
    }
    if (isSSMultiUser)
    {
        passwords.push(clientPassword);
    }
...
export function genHysteriaLink(input: GenHysteriaLinkInput): string
{
    ...
    params.set('security', 'tls');
    const tls = stream.tlsSettings;
    if (tls.settings.fingerprint.length > 0)
    {
        params.set('fp', tls.settings.fingerprint);
    }
    ...
    if (Array.isArray(externalProxy?.pinnedPeerCertSha256))
    {
        ...
        if (epPins.length > 0)
        {
            params.set('pinSHA256', epPins.join(','));
        }
    }
...
interface ClientShape { id?: string; security?: VmessSecurity; flow?: VlessClient['flow']; password?: string; auth?: string; email?: string }
...
export function isPostQuantumLink(link: string): boolean
{
    ...
    if (link.includes('mlkem768') || link.includes('mldsa65'))
    {
        return true;
    }
    if (link.includes('ML-KEM-768'))
    {
        return true;
    }
    return false;
