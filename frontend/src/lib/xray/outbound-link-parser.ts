...
export function parseVmessLink(link: string): Raw | null
{
    ...
    try
    {
        ...
        if (security === 'tls')
        {
            ...
            if (json.alpn)
            {
                tls.alpn = (json.alpn as string).split(',');
            }
        }
...
function parseUrlLink(link: string, expectedProto: string): URL | null
{
    try
    {
        ...
        if (url.protocol.replace(/:$/, '') !== expectedProto)
        {
            return null;
        }
        return url;
...
export function parseShadowsocksLink(link: string): Raw | null
{
    ...
    if (atIndex >= 0)
    {
        try
        {
            userInfo = Base64.decode(core.slice('ss://'.length, atIndex));
        }
        catch
        {
            userInfo = core.slice('ss://'.length, atIndex);
        }
