...
// --- MTProto (Telegram) FakeTLS secret helpers ---
// The "ee" FakeTLS secret is: the "ee" marker, 16 random bytes (32 hex chars),
// then the FakeTLS domain encoded as hex. The backend re-derives it on save
// (Go model.GenerateFakeTLSSecret / HealMtprotoSecret); these mirror it so the
// form can show a valid secret immediately.
...
// Rewrite only the domain suffix of an existing secret, preserving its 16-byte
// random middle when valid (generating a fresh one otherwise), so editing the
// FakeTLS domain doesn't needlessly rotate the secret's identity.
