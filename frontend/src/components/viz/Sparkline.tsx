...
}: SparklineProps)
{
    ...
    const yDomain = useMemo<[number, number]>(() =>
    {
        ...
        {
            if (Number.isFinite(p.value) && p.value > max)
            {
                max = p.value;
            }
            if (hasSeries2 && Number.isFinite(p.value2) && p.value2 > max)
            {
                max = p.value2;
            }
            if (hasSeries3 && Number.isFinite(p.value3) && p.value3 > max)
            {
                max = p.value3;
            }
        }
    ...
    const yTicks = useMemo(() =>
    {
        ...
        {
            const out: number[] = [];
            for (let v = min; v <= max; v += yTickStep)
            {
                out.push(v);
            }
            return out;
    ...
    {
        if (!extrema?.show || multiSeries || points.length < 2)
        {
            return null;
        }
        let minIdx = 0;
    ...
    const pillClass =
    'pointer-events-none absolute top-0.5 end-2 z-[1] inline-flex items-center gap-3 rounded-full border border-border px-2 py-0.5 text-[11px] font-semibold leading-4';
    const pillStyle = { background: 'color-mix(in srgb, var(--surface-raised) 88%, transparent)' };
    const itemClass = 'inline-flex items-center gap-1 whitespace-nowrap';

    return (
    <div className="relative w-full">
      {extremaPoints && (
        <div className={pillClass} style={pillStyle} aria-hidden="true">
          <span className={itemClass} style={{ color: maxColor }}>
            ▲ {fmtExtrema(extremaPoints.max.value)}
          </span>
          <span className={itemClass} style={{ color: minColor }}>
      ...
      {legendItems.length > 0 && (
        <div className={pillClass} style={pillStyle} aria-hidden="true">
          {legendItems.map((s) => (
            <span key={s.name} className={itemClass} style={{ color: s.color }}>● {s.name}</span>
          ))}
        </div>
      )}
      <ResponsiveContainer width="100%" height={height} className="block w-full">
        <AreaChart
          data={points}
          margin={{
              top: showAxes ? 14 : 6,
        ...
        >
          ...
          <YAxis
            domain={yDomain}
            hide={!showAxes}
            tick={{ fontSize: 10, fill: 'var(--muted-foreground)', dx: -4 }}
            axisLine={false}
            tickLine={false}
            tickMargin={8}
          ...
          {showTooltip && (
            <Tooltip
              ...
              itemStyle={{ color: 'var(--foreground)', padding: 0, fontWeight: 500 }}
