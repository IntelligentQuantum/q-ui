...
async function fetchOutboundsTraffic(): Promise<OutboundTrafficRow[]>
{
    ...
    if (!msg?.success)
    {
        throw new Error(msg?.msg || 'Failed to fetch outbounds traffic');
    }
...
export function useXraySetting(): UseXraySettingResult
{
    ...
    const resetTrafficMut = useMutation({
        ...
        onSuccess: (msg) =>
        {
            if (msg?.success)
            {
                queryClient.invalidateQueries({ queryKey: keys.xray.outboundsTraffic() });
            }
        }
    });
    ...
    const restartMut = useMutation({
        mutationFn: async () =>
        {
            ...
            {
                setRestartResult(validated.obj || '');
            }
            return msg;
        }
    });
    ...
    const testAllOutbounds = useCallback(async (mode = 'tcp') =>
    {
        ...
        try
        {
            ...
            list.forEach((ob, i) =>
            {
                ...
                if (proto === 'blackhole' || proto === 'loopback' || tag === 'blocked')
                {
                    return;
                }
                if (mode === 'tcp' && (proto === 'freedom' || proto === 'dns'))
                {
                    return;
                }
