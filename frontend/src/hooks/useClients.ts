...
): ClientsSummary
{
    ...
    for (const c of stats)
    {
        ...
        if (c.enable && onlineSet.has(email))
        {
            online.push(email);
        }
        if (exhausted || expired)
        {
            depleted.push(email); continue;
        }
        if (!c.enable)
        {
            deactive.push(email); continue;
        }
...
function buildQS(p: ClientQueryParams): string
{
    ...
    if (p.protocol)
    {
        sp.set('protocol', p.protocol);
    }
    if (p.inbound)
    {
        sp.set('inbound', p.inbound);
    }
    if (p.sort)
    {
        sp.set('sort', p.sort);
    ...
    if (p.expiryTo && p.expiryTo > 0)
    {
        sp.set('expiryTo', String(p.expiryTo));
    }
    if (p.usageFrom && p.usageFrom > 0)
    {
        sp.set('usageFrom', String(p.usageFrom));
    }
    if (p.usageTo && p.usageTo > 0)
    {
        sp.set('usageTo', String(p.usageTo));
    }
    if (p.autoRenew)
    {
        sp.set('autoRenew', p.autoRenew);
    }
    if (p.hasTgId)
    {
        sp.set('hasTgId', p.hasTgId);
    }
    if (p.hasComment)
    {
        sp.set('hasComment', p.hasComment);
    }
    if (p.group)
    {
...
async function fetchClientPage(params: ClientQueryParams): Promise<ClientPageResponse>
{
    ...
    if (!validated.obj)
    {
        throw new Error('Empty clients response');
    }
