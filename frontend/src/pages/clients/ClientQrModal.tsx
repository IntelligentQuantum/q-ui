...
import { Accordion, Modal, Spinner } from '@/components/ui';
...
}: ClientQrModalProps)
{
    ...
    {
        if (!client?.subId || !subSettings?.enable || !subSettings?.subURI)
        {
            return '';
        }
    ...
    {
        if (!client?.subId || !subSettings?.enable)
        {
            return '';
        }
    ...
    >
      ...
      {!client?.subId && !loading && (
        <div className="py-6 text-center text-sm text-muted-foreground">{t('pages.clients.noSubId')}</div>
      )}
      {client?.subId && !hasAnything && !loading && (
        <div className="py-6 text-center text-sm text-muted-foreground">{t('pages.clients.noLinks')}</div>
      )}
      {!loading && hasAnything && (
        <Accordion items={items} defaultActiveKeys={items[0] ? [items[0].key] : []} />
      )}
    </Modal>
    );
}
